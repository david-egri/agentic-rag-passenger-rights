...
## 2026-06-04 — Phase 5 spot-check: two 3B misclassifications surfaced while curating Agent-tab examples  (`spot-check-misroutes`)
**[gotcha]** Expanded the Agent-tab example questions from 5 → 12 to walk every graph direction (both rights_info paths, all three distance bands, sub-threshold €0, denied boarding, 50% reduction, mixed with both eligibility verdicts, two out_of_scope). Each shipped example was **run through `agent_graph` to confirm it routes/computes as intended** (the examples are now verified, not just plausible — [streamlit_app.py](streamlit_app.py) `render_agent_tab`). Doing that verification surfaced **two genuine correctness gaps**, both 3B-model limitations, recorded here for the Phase 6 eval rather than fixed now (user's call):
- **🔖 `ATC-NOT-EXTRAORDINARY` — eligibility wrongly judges ATC restrictions compensable.** "…delayed 4 h due to **air traffic control restrictions**" → the eligibility LLM returned `eligible=True` → **€400**, but ATC is an **extraordinary circumstance** that should gate to **€0** (CLAUDE.md gotcha; the same node correctly handles weather/snowstorm → €0 and own-staff strike → €250). So the [[eligibility-control-frame]] control-test prompt + few-shot covers weather/strikes but **misses ATC/airspace** — likely needs ATC (and security/third-party) added to the "beyond the carrier's control" examples.
- **🔖 `SCOPE-ASYMMETRY-MISROUTE` — EU-route-asymmetry question misrouted to fallback.** "Am I covered flying **New York → Paris on a US airline**?" → intake classified it **out_of_scope** → fallback, so the asymmetric-scope coverage question (non-EU→EU on a non-EU carrier = *not* covered; CLAUDE.md route-scope gotcha) never reaches RAG. The 3B intake reads "New York / US airline" as off-topic. Likely needs the intake prompt to treat *coverage/scope* questions as in-scope (`rights_info`).
**Why not fix now:** both are model-quality issues in the [[rag-grader]] / [[eligibility-control-frame]] family; the proper place to measure + tune them is the Phase 6 functional eval (15-Q set + methodology), not ad-hoc prompt-poking during example curation. Dropped the two bad questions (and one — meals/accommodation — that classified `mixed` and ran the calculator with no route) from the examples so the shipped set is all-correct.
**Revisit (Phase 6 eval):** add ATC + the EU-route-asymmetry case to the 15-Q set; if they still fail, extend the eligibility few-shot (ATC/security/third-party → not in control) and the intake prompt (scope/coverage questions → `rights_info`), or bump model size per [[llm-model]]. Grep `ATC-NOT-EXTRAORDINARY` / `SCOPE-ASYMMETRY-MISROUTE`.
Related: [[eligibility-control-frame]], [[rag-grader]], [[llm-model]], [[metro-aliases]].

## 2026-06-04 — `trace` is a custom domain artifact in state, not native LangGraph tracing  (`trace-in-state`)
**[decision]** The per-node "agent steps" record is a **custom `trace` field on `AgentState`** (`Annotated[list, operator.add]` reducer — [src/state.py](src/state.py)); every node returns `{"trace": [{"node": …, …}]}` and the append-only reducer merges them (the fan-out branches each contribute). The Agent tab renders it node-by-node (now live, as the graph streams). This is a recognized LangGraph pattern, but it's deliberately an **application-domain artifact** (a curated, presentation-shaped record of *decisions* — "router → route calculator", "1489 km → €250", "✅ compensable"), **not** observability/telemetry tracing.
**Why we hand-roll it rather than use a native mechanism:**
- **Semantic, UI-shaped headlines.** Each node emits a human-meaningful line the node author chose. LangGraph's native `stream_mode="updates"`/`"debug"` *does* expose "which node ran + its raw state delta" for free — but you'd render off raw state dicts and **couple the UI to internal state shapes**; you'd still write the interpretation code, just in the UI.
- **Decoupling + stability.** The trace is a stable presentation contract; a node can change its internals without breaking the panel.
- **Survives `.invoke()`, not just `.stream()`.** The trace lives in the final state, so the CLI runner ([src/graph.py](src/graph.py) `run_agent`) and any non-streamed path can render it. Native `updates`/`custom` streaming only exists *during* `.stream()`.
- **Fan-in handled** by the `operator.add` reducer (both parallel branches append cleanly).
**Native alternatives considered (for the record):** (a) `stream_mode="updates"`/`"debug"` — raw execution introspection, no state field; (b) `stream_mode="custom"` + `get_stream_writer()` — node emits events imperatively, *out* of state (closest native equivalent, but stream-only); (c) **LangSmith / OpenTelemetry** — observability-grade span tree with latencies/token counts, but LangSmith is hosted/paid and OTel needs a collector → **off the table** under non-neg #1 (no paid APIs) / local-first.
**[gotcha]** Minor redundancy: some trace steps re-pack fields already in state (the calculator step repeats `calc_result` fields — [src/graph.py](src/graph.py) ~L293). Accepted: the duplication buys UI stability + the semantic headline.
**[revisit] 🔖 `TRACE-VS-TIMING` (Phase 6 eval/load test)** — the custom `trace` is the **wrong tool for performance numbers**; don't overload it with latencies. When Phase 6 needs the per-node **timing**/bottleneck story (CLAUDE.md load-test goal; [[drop-dummy-llm]]), layer in native `stream_mode="debug"` (or `"updates"` + timestamps) *alongside* the semantic trace rather than stuffing durations into it. Decide then whether timing surfaces in the UI or only in the load-test report. Grep `TRACE-VS-TIMING` to jump back here.
Related: [[agent-assembly]], [[ui-tabs-per-layer]], [[drop-dummy-llm]].
